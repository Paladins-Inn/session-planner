...
As example we show the domain of `session-planner` but the same pattern is applied to every other domain with data lead, too.
...
. The java library `de.paladins-inn.torganized-play:session-planner-store` needs to be added to the dependency list of the client.
The `de.paladins-inn.torganized-play:session-planner-client` will be automatically included.
...
together {
  ...
  component "session-planner" as scsUser << External >>
}
...
. The java library `de.paladins-inn.torganized-play:session-planner-client` needs to be added to the dependency list of the client.
